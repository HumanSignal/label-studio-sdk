...
async def test_create(client: LabelStudio, async_client: AsyncLabelStudio) -> None:
    ...
    response = client.export_storage.s3s.create(role_arn="role_arn", project=1)
    validate_response(response, expected_response, expected_types)

    async_response = await async_client.export_storage.s3s.create(role_arn="role_arn", project=1)
    validate_response(async_response, expected_response, expected_types)
...
async def test_validate(client: LabelStudio, async_client: AsyncLabelStudio) -> None:
    # Type ignore to avoid mypy complaining about the function not being meant to return a value
    assert (
        client.export_storage.s3s.validate(role_arn="role_arn", project=1)  # type: ignore[func-returns-value]
        is None
    )

    assert (
        await async_client.export_storage.s3s.validate(role_arn="role_arn", project=1)  # type: ignore[func-returns-value]
        is None
    )
