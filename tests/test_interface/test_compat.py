...
def test_get_first_tag_occurence_complex():
    conf = LabelInterface(c.SIMPLE_CONF)
    from_name, to_name, value = conf.get_first_tag_occurence("Choices", ("Image", "Text"),
    ...
    with pytest.raises(ValueError):
        conf.get_first_tag_occurence("Choices", ("Image", "Text"),
                                     name_filter=lambda s: s.startswith("wrong_prefix"))
    
    with pytest.raises(ValueError):
        conf.get_first_tag_occurence("Choices", ("Image", "Text"),
                                     to_name_filter=lambda s: s == "wrong_name")
