# This file was auto-generated by Fern from our API Definition.

import typing

import pydantic
from ..core.pydantic_utilities import IS_PYDANTIC_V2
from ..core.unchecked_base_model import UncheckedBaseModel
from .prepare_params_filters_request import PrepareParamsFiltersRequest
from .prepare_params_ordering_request import PrepareParamsOrderingRequest


class ViewDataRequestRequest(UncheckedBaseModel):
    """
    Established public view payload nested under ``data``.
    """

    filters: typing.Optional[PrepareParamsFiltersRequest] = None
    ordering: typing.Optional[PrepareParamsOrderingRequest] = None

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
